...
#include "shr_assert.h"
  ...
  type, public :: photosyns_type
     ...
     ! Haoran Liu Add CVF  
     real(r8), pointer, public  :: cvf_patch         (:)   ! Conversion Factor Constant	 
     ! Haoran Liu Add CVF 
     ...
     real(r8), pointer, public  :: SIFsun_patch      (:)   ! patch sunlit leaf SIF     
     real(r8), pointer, public  :: SIFsha_patch      (:)   ! patch shaded leaf SIF     
     ...
     real(r8), pointer, private :: SIFsun_z_patch    (:,:) ! patch canopy layer: sunlit leaf SIF   (umol CO2/m**2/s)
     real(r8), pointer, private :: SIFsha_z_patch    (:,:) ! patch canopy layer: shaded leaf SIF   (umol CO2/m**2/s)	 
     real(r8), pointer, private :: lSIFsun_z_patch   (:,:) ! patch leaf level: sunlit leaf SIF   (umol CO2/m**2/s)
     real(r8), pointer, private :: lSIFsha_z_patch   (:,:) ! patch leaf level: shaded leaf SIF   (umol CO2/m**2/s)	     
     ...
     real(r8), pointer, public  :: fSIF_patch		  (:)   ! patch Leaf-level SIF HaoranLiu	 
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    !------Conversion Factor-----Haoran Liu
    allocate(this%cvf_patch         (begp:endp))           ; this%cvf_patch         (:)   = nan	
    !------Conversion Factor-----Haoran Liu
    ...
    allocate(this%lSIFsun_z_patch   (begp:endp,1:nlevcan)) ; this%lSIFsun_z_patch   (:,:) = nan
    allocate(this%lSIFsha_z_patch   (begp:endp,1:nlevcan)) ; this%lSIFsha_z_patch   (:,:) = nan	    
...
!    allocate(this%lmrcanopy_patch   (begp:endp))           ; this%lmrcanopy_patch   (:)   = nan
  ...
  subroutine InitHistory(this, bounds)
    ...
    if (.not. use_fates) then
	   ...
	   ! Export SIF Haoran Liu
       this%fSIF_patch(begp:endp) = spval
       call hist_addfld1d (fname='SIF', units='umol m-2 s-1',  &
  ...
  subroutine InitCold(this, bounds)
    ...
    do p = bounds%begp,bounds%endp
       l = patch%landunit(p)
      ! ----------- Haoran Liu get Conversion Factor based on the land cover types --------
       if (l == 1)   this%cvf_patch(p) = 1.0_r8
       if (l == 2)   this%cvf_patch(p) = 1.0_r8
       if (l == 3)   this%cvf_patch(p) = 1.0_r8
       if (l == 4)   this%cvf_patch(p) = 1.0_r8
       if (l == 5)   this%cvf_patch(p) = 1.0_r8
       if (l == 6)   this%cvf_patch(p) = 1.0_r8
       if (l == 7)   this%cvf_patch(p) = 1.0_r8
       if (l == 8)   this%cvf_patch(p) = 1.0_r8
       if (l == 9)   this%cvf_patch(p) = 1.0_r8
       if (l == 10)  this%cvf_patch(p) = 1.0_r8
      ! ----------- Haoran Liu get CVF based on the land cover types --------
  ...
  subroutine Photosynthesis ( bounds, fn, filterp, &
	...
	! Export results of SIF HaoranLiu 
    use histFileMod   , only: hist_addfld1d, hist_addfld2d
    ...
    real(r8) :: kp      
    real(r8) :: kk 
    real(r8) :: kn_p !kn already exists in the code, so add a _p to distiguish them 	
    real(r8) :: fm		
    real(r8) :: ps_sif      
    real(r8) :: ffs 
    real(r8) :: stSIF_z 
    ...
    associate(                                                 &
         ...
         clumping_sif   => pftcon%clumping_sif               , & ! Input:  ecophys const - clumping index for SIF calculation     
         !Haoran Liu SIF----End	
      ...
      if (phase == 'sun') then
         ...
         SIF       =>    photosyns_inst%SIFsun_patch         ! Output: [real(r8) (:)   ]  foliage SIF (umol co2 /m**2/ s) [always +]
         psn_z     =>    photosyns_inst%psnsun_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage photosynthesis (umol co2 /m**2/ s) [always +]
         lSIF_z    =>    photosyns_inst%lSIFsun_z_patch      ! Output: [real(r8) (:,:) ]  leaf level:  SIF (umol co2 /m**2/ s) [always +]         
         SIF_z     =>    photosyns_inst%SIFsun_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage SIF (umol co2 /m**2/ s) [always +]
         ...
         SIF       =>    photosyns_inst%SIFsha_patch         ! Output: [real(r8) (:)   ]  foliage SIF (umol co2 /m**2/ s) [always +]
         psn_z     =>    photosyns_inst%psnsha_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage photosynthesis (umol co2 /m**2/ s) [always +]
         lSIF_z    =>    photosyns_inst%lSIFsha_z_patch      ! Output: [real(r8) (:,:) ]  leaf level: foliage SIF (umol co2 /m**2/ s) [always +]
         SIF_z     =>    photosyns_inst%SIFsha_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage SIF (umol co2 /m**2/ s) [always +]
      ...
      do f = 1, fn
         ...
         do iv = 1, nrad(p)
            ...
            if (par_z(p,iv) <= 0._r8) then           ! night time
               ...
               !Haoran Liu 2020.07.07
               !----------------------Leaf Level SIF Calculation Start-----------------------		
               sif_je = psn_z(p,iv) * (ci_z(p,iv) + 2.0_r8 * cp(p))/(ci_z(p,iv) - cp(p))
               if (sif_je < 0._r8) then 			   
                  sif_je = 0._r8
               end if
               sif_par = par_z(p,iv)/4.6_r8 ! unit conversion: 1 W/m**2 = 4.6 umols/m**2 * s
               xxn = 1.0_r8 - sif_je/(sif_par * 0.05_r8)
               if (xxn < 0._r8) then 			   
                  xxn = 0._r8
               end if
               ...
               ps_sif = kp/(kf+kp+kd)*(1.0_r8-xxn);
               kk=exp(log(xxn)*2.83_r8);
               kn_p = 2.48_r8*(1.0_r8+0.114_r8)*kk/(kk+0.114_r8);
               fm = kf/(kf+kd+kn_p);
               ffs = fm*(1.0_r8-ps_sif);
               lSIF_z(p,iv) = ffs * sif_par
               !----------------------Leaf Level SIF Calculation END-----------------------
      ...
      do f = 1, fn
         ...
         do iv = 1, nrad(p)
            psncan = psncan +psn_z(p,iv) *lai_z(p,iv)
			   !----------------------Canopy Level SIF Calculation Start-----------------------		  
            ...
            clumping_index_sif = max(0.001_r8, clumping_sif(p))		
                        
			   AA = 0.5_r8 - 0.633_r8 *chil - 0.330_r8 *chil *chil
			   BB = 0.877_r8 *(1._r8-2._r8 *AA)
            GMU = (AA + BB *cosz) /cosz 
            stSIF_z = lSIF_z(p,iv) *0.5_r8 *clumping_index_sif *(1.1_r8 -0.1_r8 *lai_z(p,iv)) *exp(-1.0_r8 *cosz) *lai_z(p,iv)
            SIF_z(p,iv) = lSIF_z(p,iv) *exp(-GMU *clumping_index_sif *lai_z(p,iv) /cosz) *lai_z(p,iv) +stSIF_z            
            SIFcan = SIFcan + SIF_z(p,iv)	
			   !----------------------Canopy Level SIF Calculation End-----------------------       
  ...
  subroutine PhotosynthesisTotal (fn, filterp, &
    ...
    associate(                                             &
         ...
         SIFsun      => photosyns_inst%SIFsun_patch      , & ! Input:  [real(r8) (:) ]  sunlit leaf photosynthesis (umol CO2 /m**2/ s)
         SIFsha      => photosyns_inst%SIFsha_patch      , & ! Input:  [real(r8) (:) ]  shaded leaf photosynthesis (umol CO2 /m**2/ s)		 
         cvf         => photosyns_inst%cvf_patch         , & ! Output: [real(r8) (:) ]  conversion factor
         ...
         fSIF        => photosyns_inst%fSIF_patch        , & ! Output: [real(r8) (:) ]  SIF (umol CO2 /m**2 /s) Haoran Liu		 
      ...
      do f = 1, fn
         ...
         cvf_index = -1.0_r8 * log(laisun(p) + laisha(p)) + cvf(p);
         if (.not. use_fates) then
            fpsn(p)    = psnsun(p)   *laisun(p) + psnsha(p)   *laisha(p)
			   fSIF(p)    = (SIFsun(p)  + SIFsha(p)) /cvf_index	! total SIF
  ...
  subroutine PhotosynthesisHydraulicStress ( bounds, fn, filterp, &
    ...
    real(r8) , pointer :: SIF_sun         (:)   ! foliage SIF, sunlit (umol co2 /m**2/ s) [always +]	
    ...
    real(r8) , pointer :: SIF_z_sun       (:,:) ! canopy layer: foliage SIF, sunlit (umol co2 /m**2/ s) [always +]		
    ...
    real(r8) , pointer :: SIF_sha         (:)   ! foliage SIF, shadelit (umol co2 /m**2/ s) [always +]		
    ...
    real(r8) , pointer :: SIF_z_sha       (:,:) ! canopy layer: foliage SIF, shaded (umol co2 /m**2/ s) [always +]		
...
!!!
      ...
      SIF_sun       =>    photosyns_inst%SIFsun_patch         ! Output: [real(r8) (:)   ]  foliage photosynthesis (umol co2 /m**2/ s) [always +]	  
      psn_z_sun     =>    photosyns_inst%psnsun_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage photosynthesis (umol co2 /m**2/ s) [always +]
      SIF_z_sun     =>    photosyns_inst%SIFsun_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage photosynthesis (umol co2 /m**2/ s) [always +]
      ...
      SIF_sha       =>    photosyns_inst%SIFsha_patch         ! Output: [real(r8) (:)   ]  foliage photosynthesis (umol co2 /m**2/ s) [always +]
      psn_z_sha     =>    photosyns_inst%psnsha_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage photosynthesis (umol co2 /m**2/ s) [always +]
      SIF_z_sha     =>    photosyns_inst%SIFsha_z_patch       ! Output: [real(r8) (:,:) ]  canopy layer: foliage photosynthesis (umol co2 /m**2/ s) [always +]
