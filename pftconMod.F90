module pftconMod
  ...
  type, public :: pftcon_type
     ...
     ! Haoran Liu Start 
     real(r8), allocatable :: clumping_sif  (:)   ! vegetation clumping index for SIF calculation 
     ! Haoran Liu End 
...
contains
  ...
  subroutine InitAllocate (this)
    ...
    !Start  Haoran Liu
    allocate( this%clumping_sif  (0:mxpft) ) 
    !End  Haoran Liu          
  ...
  subroutine InitRead(this)
    ...
    ! ---- Add Clumping Index  for SIF --- Start  Haoran Liu ----
    call ncd_io('clumping_sif', this%clumping_sif, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(sourcefile, __LINE__))
    ! ---- Add Clumping Index for SIF --- End  Haoran Liu ----
